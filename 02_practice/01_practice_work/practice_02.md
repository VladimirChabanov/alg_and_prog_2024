...
1. В каталоге `projects` создайте каталог `lab2` и перейдите в него;
2. Откройте терминал в каталоге `lab2` (текущий каталог должен быть `projects/lab2`);
3. Наберите команду: `code .`;
4. Активируйте профиль *Remote* и добавьте конфигурацию *Dev Container* для разработки под С++ в текущий каталог. В качестве ОС контейнера выберите *Ubuntu 24.04*;
...
42. Выполните команду и внимательно изучите вывод:
    ...
    Здесь намеренно нет ключа `-C`, чтобы выводились настоящие (манглированные) имена. Теперь можно заметить, что в файле `"foo.o"` не определён символ `second`, а в `"main.o"` такого нет. Место этого там `_Z6secondv`.
