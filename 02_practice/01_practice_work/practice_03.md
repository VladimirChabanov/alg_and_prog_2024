...
Но данный способ, помимо того, что требует ввода 3-х команд ещё и требует ввода полной информации о коммите, иначе данные будут текущие.

Есть способ проще - использовать ключ `--amend`.
...
129. Выполните команду:
     ...
     Обратите внимание, что текст коммита, *Author* и *AuthorDate* явно взяты из заменённого коммита (особенно это видно по дате), но кроме этого, есть ещё два поля *Commit* и *CommitDate* которые равны текущим параметрам.

     Дело в том, что с коммитом ассоциируется два человека: автор и коммитер. Когда вы создаёте коммит, то эта информация совпадает, но когда коммит "модифицируется" информация о коммитете и дате его "изменения" заменяется на текущую.

     Изменить ифнормацию о коммитетре через ключи нельзя, но можно воспользоваться переменнымим оккужения:
     ...
     Это всё одна строка, просто разделённая для удобства символом переноса `\`. Значения для переменных - это результат выполнения соответствующих команд.

     Точно также, при помощи `--amend` можно заменить только информацию коммите, не трогая содержимое (например, если обнаружили опечатку в тексте сообщения или закоммитили не с того аккаунта):
...
Далее мы ещё затроним операцию перебазирования, но сейчас мы воспользуемся ей для удаления части коммитов которые находятся в середине истории - *2-го*  и *3-го*. Для этого, просто переставим 4-й и 5-й коммит на первый
...
133. Удаление коммитов:
     ...
     - В открывшемся редакторе замените pick на drop у коммита с мтекой 2 и 3;
...
134. Склеивание коммитов (сквош):
     ...
     - В открывшемся редакторе замените pick на squash у коммита с мтекой 3 и 4 и 5; Второй коммит нельзя заменить на squash, т.к. должен быть предыдущий.

     - Сохраните  и закройте файл

       В результате откроется файл в котором нужео будет написать сообщение. 

     - Сотрите все и оставьие только 2-5. Сохраните  и закройте файл

     - Посмотрите историю.

       Как видно, осталось только 2 коммита первый и второй содержаий все последующие изменения.

     - Выполните *hard reset* обратно на тег *start*;

135. Исправление (сквош).

     - Выполните теже действия, что для для сквоша, но squash замените на fixup. В результате, после того, как вы зактроете первый редатор коммит будет создан автоматически с сообщением из второго коммита. Это полезно, если вы сделали коммит и тут же обнаружи ли несколько багов которые закоммитили отдельно. Чтобы об этом не было упоминаний в истории можно их просто схлопнуть.

     - Выполните *hard reset* обратно на тег *start*;

136. Изменение порядка комитов:
     ...
     - В открывшемся редакторе замените поменяйте местаим строки с сообщение 5 и 2

     - Сохраните  и закройте файл

     - Посмотрите историю.

       Как видно, порядок омииторв теперб ихменился.

     - Выполните *hard reset* обратно на тег *start*;

137. Изменения коммит меседжа:
     ...
     - В открывшемся редакторе замените pick на reword у коммита с мтекой 4;

     - Сохраните  и закройте файл

       В результате откроется текстовый файл в котором нужео будет написать новое сообщение к комиту 4. 

     - Напмимште: new 4. Сохраните  и закройте файл

     - Посмотрите историю.

     - Выполните *hard reset* обратно на тег *start*;

138. Изменение/разделение коммита:
     ...
     - В открывшемся редакторе замените pick на edit у коммита с мтекой 4;

     - Сохраните  и закройте файл

       В результате процесс перебазирования остановится после применения 4-го коммита о чём свидетельствует сообщение в строке ввода мы находимся на 3м шаге перебазирования из 4х. Теперь мы может использовать `--amend`, чтобы внести в него изменение.

       Если вы хотите, то можете создать несколько новых коммитов. Например, если в коммит попали несколько файлов, можно убрать лишний из кэша сделать коммит, а затем закоммитаить второй файл.
     ...
     - И продолжите перебазированеи командой:
     ...
     - Выполните команду:
       ...
       Как видно в 4 коммите было добавлено 2 файла.

     - Выполните *hard reset* обратно на тег *start*;

Не всегда перебазирование происходит так гладко, довольно часть можно попасть в состояние конфликат. В этом случае перебазированеи можно преравать и вернуться к начальному состоянию командой `git rebase --abort`.

### Отмена изменений

При совестной работе любое переписывание истории чревато проблемами. Поэтому существует способ отменить изменение в коммите или нескольких без переписывания истории, а создавая новый коммит, который отменяет внесённые изменения.
...
139. Выполните команду:
     ...
     В результате, рабочий каталог и кэш придут в состояние как буд-то бы коммиты 4-й (`master~1`) и 2-й (`master~3`) не создавались. Ключ `--no-commit` предотвращает автоматический запуск команды `git commit`.

     Кроме того,как видно по строке приглашения к вводу видно, мы находимся в прометоточном состоянии . Если нас что-то не устаривает можно отменить `revert` командой `git revert --abort`.

140. Создайте коммит с сообщением: `"Отмена 2 и 4"`.

     После созадния коммита вы выйдите из промеюутончого состояни.

141. Посмотрите историю.

     Как видно, в истории появился новый коммит, а старые остались на месте. Именно поэтому важно не смешивать в коммитах изменения в несвязных местах. Иначе отменить их будет довольно не просто.

### Слияние изменений из разных веток

![](./practice_03_img/git_merge.png)

Ранее мы рассмотрели работу с веткаим. В больших проектах векти как правило создают для реализации отдельной фичи и затем функционал из веки добавляется (вливается) в основную или dev ветку.
...
Гит производит, так называемое, трёхстороннее слияние, т.е. в процессе слияния учувствуют три коммита: последний коммит из текущей ветки, последний коммит из вливаемой ветки и база - общий коммит на котором история двух веток разошлась (слияние одноверменно больше чем двух веток мы рассматривать не будет, хотя это возможно).

В результате слияниея создаётся коммит содержищий все изменения из обоих веток. В отличие от обычных коммитов у коммита-слияния два (и более) родителя: первый родитель это коммит из текущей ветки, второй - из вливаемой.
...
143. Выполние команды:
     ...
     Как видно, здесь создаётся новый файл и перезаписываем task5.txt текста. После чего создаётся коммит.

144. Перейдите в ветку *master* и выполните команду:
     ...
     Первое правило: если относительно базового коммита изменения в файле выполнялись только в одном коммите, то в коммит-слияния попадёт эта версия. Дата-время коммитов не учитывается, только факт отличия от базового коммита

145. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*:
...
146. Выполните команды:
     ...
     Откроется тексовый редатор предалгающий ввести коммит месседж просто закройте. Стандартный нас устраивает.

     Здесь мы изменяем файл task3.txt, создаём коммит и сливаем в текущую ветку изменения из ff.

     Теперь стратегию перементки применить не пуолчается, т.к. присутствуют изменения в обеих ветках и если выполните перемотку, то пропадут изменения из ветки master, поэтому git выполняет автоматическо слияение и создаёт коммит.
...
147. Выполните команды:
     ...
     Как видно, текущий коммит включает изменения из обеих векток. Гит действует по правилу описанному ранее. Т.е. файл task3.txt изменялся только в ветке master, поэтому в коммит попал он, а task5.txt только в ветке ff, поэтому он взят оттуда. Отсальные файл не отличаются от базового коммита, поэтому они одинаковые.

148. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*.

149. Создайте ветку conf и перейдите в неё.

150. Переименуйте файл task1.txt в task_conf.txt и и закомитте изменения/

151. Перейдите в ветку master и переименуйте файл task1.txt в task_master.txt и тоже закоммитьте.

152. Выполните слияние master с conf.

     Может показаться, что всё должно быть успешно и в коммит должны попасть оба файла, но git действительно соаздёт оба файла, но сообщает о конфликте из-за того, что обе ветки хотят переименовать один и тот же файл (относительно базового коммита).

     Выполните команду, чтобы отменить слияние:

     ```bash
     git merge --abort
     ```

153. Повторно запустите слияние.

     В результате вы снова попадёте в состояние конфликта. Гит будет ждать пока разрабочик не примет решение и не создаст коммит. Здесь мы можем делать что угодно, главное потом создать коммит.

     Предположим, что мы решаем оставить оба файла, тогда выполните команды:
     ...
     После чего вы видите, что сообщение о прерванном слиянии сменлось просто на имя ветки.
...
154. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*.

155. Создайте ветку same и прейдите в неё.

156. Допишите в файл task2.txt текст: *"Изменения из ветки same"* **на новой строке**. И создайте коммит.

157. Переключитесь обратно на ветку *master* и допишите в файл task2.txt текст: *"Изменения из ветки master"* **на новой строке**. И создайте коммит.

     Таким образов в файле task2.txt произведены разные изменения во второй троке в разных ветках.

158. Выполните слияние master с same.

     Ожидаемо произошёл конфликт при слиянии файла  task2.txt. Гит не знаект версию из какой ветки брать. Но git всё равно попыткался выполнить слияние и оставил соответствующие пометки там, где у него это не получилось/

159. Выведите содерживое файла task2.txt в терминал:

     ```bash
     cat task2.txt
     ```

     Как видно, первая строка в обеих ветках совпадает, поэтому с ней проблем нет, а вот при сляинии второй строки етсь два вариата, поэтому git показал их разделив соовтетствующим маркерами:
     ...
     Это вариант второй строки их текущей ветки
     ...
     Далее git ждёт, что мы примем какое-нибуь решение относительно таких вот блоков во всех файлах в которых есть конфликт. В принуипе git без разницы, что мы следатем. Можем оставить первый, вариант или второй, или оба сразу или удать всё и т.д. главное после этого добавить файл в кэщ сделать коммит.

160. Откройте файл task2.txt и Замените блок
     ...
     Т.е. мы решаем оставить оба изменения, но в драгуом порядке.

     Обновите файл в кэше и создайте коммит.

     Как видно, мы вышли из состояния слияния.

161. Не всегда удобно ориентироваться по таким вот разделителям которые оставил git, чтобы они были нагляднее настроим внешний иснтрумент сляиния, так же ка мы это делали для diff.
     ...
     Здесь мы назначаем в качестве внешнего инструмента слияния vscode. Последняя строка нужна, чтобы не создавался бэкап конфликтных файлов (обычно он создаётся, при использовании внешнего иснтумента слияния).

162. Выполнте команды:
     ...
     В результате мы откатимся назад отменив коммит слияния и снова выполним слияние с веткой same и разумеентся опять попадём в конфликт. Теперб мы будем разрешать кофнликт не руками, а примпомощи vscode.

     В результате запуска внешенго инстумента, в рабочем каталоге будут создано 4 временных файлв, которыу будут удалены после слияния. и откроется коно vs code.

163. В открывшемся окне установите галочку *Show Base Center*, таким образов у вас получится три окна вверху и одно снизу по центру.

     ![](./practice_03_img/vscode_mergetool.png)

     Сверху будут показаны документы в порядке: из вливаемой ветки, затем база, затем из текущей ветки. Снизу показан реазультрующий файл, который войдёт в коммит.

     Таким образом мы наглядно видим, что строка 1 в обоих документах одинаковая и совпадает с базой. Строка 2 отличается и в базе её не было.

     При помощи мыши нажимте на копку Accept в левом окне, а затем в правом. При необходимости можно откатить изсенения или поправить результат в нижнем окне.

164. Сохраните изменения и закройте окно и на вопрос в терминале об удачном решении конфликат нажмите y.

     Обновите файл в кэше и завтериште слияние коммитом.

Не всегда изменение в одной и той же строке приведут к конфликту слияния. Гит старается автоматически объединить файлы если это возможно. Для этого git делит файл на блоки и если изменение прлизведено в одном блоке, то это конфликт, если в разынх, то в итоговый файл попадут оба изменения.

165. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*.

166. Создайте файл index.html содержищий:
...
167. Созадйте ветку html и прейдите в неё. Затем мзените содержимое файлва  index.html на следубщее:
...
168. Вернитесь в ветку master и мзените содержимое файлва  index.html на следубщее:
...
169. Выполните слияние с втекой html.

     В результате вам сразу предложат вветси сообщение для коммита. И никаокго конфдика тне будет.

170. Посмотерте содержиме файла index.html

     ```bash
     cat index.html
     ```

     Как видно, изменения из обоих веток присутутвуют в итоговом файле.

     Такое произошло, т.к. git использовал низменяемые относительно базы строки `<tag>` и border как границы первого блока и dorder и `</tag>` как границы второго. Таким образом с точки зрения git изменения в ветке html касались только первого блока,  в ветке master толкьо второго, поэтому git легко смог их объединить.
...
Таким образом история сохраняет становится менее запутанной хотя при этом и теряется информация о последовательности и разработки.

171. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*.

172. Выполните перебазирование ветки html командой:
     ...
     Как видно во время rebase git сообщил, что текущая ветка не нуждается в перебазировании. Оно и понятно, html был создан от коммита start и после этого в master никаких именения не было. Далее мы переходим на master и выолпняем слияени, которое просто передвинает указатель master на последний коммит из html

173. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*.

174. Находясь в ветке master. измените файл task1.txt дописав в конец несколько любых сиволоов и создайте коммит ["6"].

     Теперь в ветке master относительно общего коммита с html есть изменения, а значит простой перемоткой не обойтись.

175. Выполните команды:
...
176. Отменим слияние вернув указатель *master* обратно на коммита с тегом *start*.

177. Допишите в файл task2.txt текст: *"Изменения из ветки master"* **на новой строке**. И создайте коммит.

     Далее будем выполнять перебазирование вектки same на master. В ветке same присутствуют другие изменения во второй строке файла task2.txt.

178. Выполните команды:
     ...
     После команды rebase ожидаемо произошёл конфликт и процесс перебазиования остановился до момента отмены или разрешения коифликатк. 

179. Выполнте команду:

     ```bash
     git mergetool
     ```

     И разрешите кофликт оставив изменения из обоих файлов. Сохраните изменения и закройте редктор. Затем проверьте, что изменения в файле task2.txt правильные.

180. Добавьте имзенения в кэш и продолжите перебазированеи командой:

     ```bash
     git rebase --continue
     ```

     В результате будет открыт тексовый редактор чтоыб вы указаи коиит месседж исполльзуйте "rebase". Сохраните и закройте редаткр в результате будет создан коммит и перебазирование будет завершено.
...
Как видно, во многом перебазировании и слияние похоже по процессу. В обоих случаях может быть автоматическое слияние и конфликты. Но в процессе обычного слияния вы получаете как бы один суммарный конфликт или можете не получить вовсе, если итоговое состояние файлов в последнем коммите не конфлиткует с сливаемой веткой, а в процессе перебазирования все коммиты применяются по порядку, поэтому конфликтов может быть больше.
