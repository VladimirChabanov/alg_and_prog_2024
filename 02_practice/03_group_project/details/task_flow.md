...
| Посмотреть ответы пользователя                 | Для теста с указанным ID выбирает все попытки и возвращает массив, каждый элемент которого состоит из:<br />- ID пользователя выполнившего попытку;<br />- Массив ответов на вопросы этого пользователя.<br />Ответ, в данном случае - это: текста вопроса и текста ответа. | + Если пользователь преподаватель на курсе<br />+ Если пользователь смотрит свои ответы<br />- Для остальных | `test:answer:read`  |
